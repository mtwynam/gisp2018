...
import string, requests, requests_cache

requests_cache.install_cache('gettysburg-cache', backend='sqlite')
...
def main():
    ...
    try:
        ...
        speech_text = speech.text.split("\n")

        stopwords = requests.get(STOPWORDS_URL)
        stopwords.encoding = "utf-8"
        stopwords_collection = tuple(stopwords.text.strip().split(","))

        # Make word list from speech while excluding stop words
        speech_collection = make_word_list(speech_text, stopwords_collection)

        # Make a set of words from speech which automatically assures that each entry is unique
        # unique = set(word for word in speech)
        unique = set(speech_collection)

        # Print the results
        print("Speech Length: {}".format(len(speech_collection)))
        print("Unique words: {}".format(len(unique)))
        print("\nWord count")

        words = countWords(speech_collection)
        ...
        print("Request for {} returned status of {}."
              .format(SPEECH_URL, speech.status_code))
        print("Request for {} returned status of {}."
              .format(STOPWORDS_URL, stopwords.status_code))
